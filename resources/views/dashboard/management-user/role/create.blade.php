          ...
          <div class="space-y-6">
            ...
            <div>
              ...
              <div class="space-y-3">
                ...
                @foreach ($permission as $value_menu)
                  <div>
                    <h3 class="mb-4 font-semibold text-gray-900 dark:text-white text-sm capitalize">{{ $value_menu->menu }}</h3>
                    <div>
                      @foreach ($value_menu->sub_menu as $value_sub_menu)
                        <div class="ms-6">
                          <h3 class="mb-4 font-semibold text-gray-900 dark:text-white text-sm capitalize">{{ $value_sub_menu->sub_menu }}</h3>
                          <ul class="items-center w-full text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg sm:flex dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                            @foreach ($value_sub_menu->permission as $item)    
                              <li class="w-full border-b border-gray-200 sm:border-b-0 sm:border-r dark:border-gray-600">
                                <div class="flex items-center ps-3">
                                  <input id="vue-checkbox-list" type="checkbox" name="permission[]" value="{{ $value_menu->menu . '-' . $item }}" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
