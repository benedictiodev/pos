    ...
    <div
      ...
      <div class="mb-4">
        <form action="{{ route('dashboard.finance.cash-in.post') }}" method="POST">
          ...
          <div class="space-y-6">
            <div>
              <label for="fund" class="mb-2 block text-sm font-medium text-gray-900 dark:text-white">Fund</label>
            ...
            <div>
              ...
              <textarea id="remark" rows="4" name="remark"
                ...
                placeholder="Enter event remark here" required>{{ old('remark') }}</textarea>
              @error('remark')
                <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-medium">{{ $message }}</p>
              @enderror
            </div>
